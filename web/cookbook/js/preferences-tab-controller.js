...
class PreferencesTabController extends Controller {
	...
	processActivated () {
		...
		avatarImageElement.addEventListener("dragover", event => this.validateAvatarTransfer(event.dataTransfer));
		avatarImageElement.addEventListener("drop", event => this.processSubmitAvatar(event.dataTransfer.files[0]));
	...
	async processSubmitSessionOwner () {
		try {
			...
			const response = await fetch(resource, { method: "POST" , headers: headers, body: JSON.stringify(sessionOwner) });
			if (!response.ok) throw new Error("HTTP " + response.status + " " + response.statusText);

			if (sessionOwnerPassword || sessionOwner.email !== this.sharedProperties["session-owner"].email) {
	...
	async validateAvatarTransfer (dataTransfer) {
		...
		dataTransfer.dropEffect = dataTransferItem.kind === "file" && dataTransferItem.type.startsWith("image/") ? "copy" : "none";
	}
	...
	async processSubmitAvatar (avatarFile) {
		try {
			...
			if (!avatarFile.type.startsWith("image/")) throw new RangeError("illegal avatar file " + avatarFile.name + "!");
			const sessionOwner = this.sharedProperties["session-owner"];
			...
			const response = await fetch(resource, { method: "POST" , headers: headers, body: avatarFile });
			if (!response.ok) throw new Error("HTTP " + response.status + " " + response.statusText);
			sessionOwner.avatar.identity = parseInt(await response.text());

			this.center.querySelector("section.preferences img.avatar").src = "/services/documents/" + sessionOwner.avatar.identity;
